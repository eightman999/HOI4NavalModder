...
public class FleetDeploymentWindow : Avalonia.Controls.Window
{
    ...
    private readonly Bitmap _countryFlag;
    private readonly string _countryName;

    // 国家情報
    private readonly string _countryTag;
    private readonly ObservableCollection<FleetInfo> _fleetsList = new();
        ...
        try
        {
            ...
            if (navalUnits.Count > 0)
            {
                foreach (var unit in navalUnits)
                {
                    ...
                    // 港の名前を設定
                    if (_portNames.TryGetValue(unit.NavalBaseId, out var portName))
    ...
    private async void ShowFleetEditDialog(FleetInfo fleet)
    {
        ...
        foreach (var port in _portNames)
        {
            ...
            if (!isNew && port.Key == fleet.ProvinceId) baseComboBox.SelectedIndex = baseComboBox.Items.Count - 1;
        }
        ...
        saveButton.Click += (s, e) =>
        {
            try
            {
                ...
                if (baseComboBox.SelectedIndex >= 0)
                {
                    ...
                    var match = Regex.Match(selectedPortInfo, @"ID: (\d+)");
                    if (match.Success)
                    {
                ...
                else
                {
                    ...
                    // ListBoxの表示を更新
                    var index = _fleetsList.IndexOf(fleet);
                    if (index >= 0)
                    {
    ...
    private async void ShowAddShipDialog(FleetInfo fleet, ListBox shipListBox)
    {
        ...
        addButton.Click += (s, e) =>
        {
            ...
            {
                var name = nameTextBox.Text?.Trim() ?? "";
                var type = typeComboBox.SelectedItem?.ToString() ?? "不明";
                var className = classTextBox.Text?.Trim() ?? "";
                var count = (int)countNumeric.Value;
                var isPrideOfFleet = prideCheckBox.IsChecked ?? false;
    ...
    public class FleetInfo
    {
        ...
        public ObservableCollection<ShipInfo> Ships { get; set; } = new();
    }
