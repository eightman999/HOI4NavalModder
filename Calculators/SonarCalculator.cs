...
public static class SonarCalculator
{
    ...
    public static NavalEquipment Sonar_Processing(Dictionary<string, object> rawSonarData)
    {
        try
        {
            ...
            var electronics = rawSonarData.ContainsKey("Electronics")
                ? Convert.ToDouble(rawSonarData["Electronics"])
                : 0;

            // 特殊機能の取得
            var isNoiseReduction = rawSonarData.ContainsKey("IsNoiseReduction") &&
                                   Convert.ToBoolean(rawSonarData["IsNoiseReduction"]);
            var isHighFrequency = rawSonarData.ContainsKey("IsHighFrequency") &&
                                  Convert.ToBoolean(rawSonarData["IsHighFrequency"]);
            ...
            var isTowedArray = rawSonarData.ContainsKey("IsTowedArray") &&
                               Convert.ToBoolean(rawSonarData["IsTowedArray"]);
