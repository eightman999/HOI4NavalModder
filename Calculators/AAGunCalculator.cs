...
public static class AAGunCalculator
{
    ...
    public static NavalEquipment AA_Processing(Dictionary<string, object> rawAAData)
    {
        try
        {
            ...
            var hasProximityFuze = rawAAData.ContainsKey("HasProximityFuze") &&
                                   Convert.ToBoolean(rawAAData["HasProximityFuze"]);
            var hasRadarGuidance = rawAAData.ContainsKey("HasRadarGuidance") &&
                                   Convert.ToBoolean(rawAAData["HasRadarGuidance"]);
            var hasStabilizedMount = rawAAData.ContainsKey("HasStabilizedMount") &&
                                     Convert.ToBoolean(rawAAData["HasStabilizedMount"]);
            var hasRemoteControl = rawAAData.ContainsKey("HasRemoteControl") &&
                                   Convert.ToBoolean(rawAAData["HasRemoteControl"]);

            // 口径をmmに変換
            var calibreInMm = calibre;
