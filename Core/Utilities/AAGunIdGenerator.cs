...
public static class AAGunIdGenerator
{
    ...
    public static string GenerateGunId(string categoryId, string countryTag, int year, double calibre,
        string calibreType, double barrelLength)
    {
        // カテゴリIDを小文字に変換
        ...
        out double calibre, out string calibreType, out double barrelLength)
    {
        ...
        try
        {
            ...
            if (calibrePart.EndsWith("mm"))
            {
                // mm単位の場合
                calibreType = "mm";
                if (double.TryParse(calibrePart.Substring(0, calibrePart.Length - 2), out var parsedCalibre))
            ...
            if (parts.Length >= 3 && parts[2].StartsWith("l"))
            {
                ...
                if (double.TryParse(barrelPart, out var parsedBarrelLength))
            ...
            if (parts.Length >= 4)
            {
                ...
                {
                    // 国家タグがなく、年が3番目にある場合
                    if (int.TryParse(parts[3], out var parsedYear))
                ...
                else
                {
                    ...
                    // 年の抽出（存在する場合）
                    if (parts.Length >= 5 && parts[4].Length == 2 && int.TryParse(parts[4], out var parsedYear))
                        year = 1900 + parsedYear;
                }
            }
